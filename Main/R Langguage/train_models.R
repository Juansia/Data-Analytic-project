...
#  1) Loads dataset/processed_data_best_corr_sentiment.csv
#  2) Chronological split (80/10/10)
#  3) Min-max scaling using TRAIN only (avoids leakage)
#  4) Builds rolling sequences for LOOKBACK -> predict HORIZON days ahead
#  5) Trains selected model with early stopping
#  6) Saves metrics + test predictions + model to outputs/
#
# Notes:
#  - Metrics are saved both on SCALED target and ORIGINAL price scale (inverse transformed).
#  - For "SENT-USDX-Encoder-decoder-GRU" your CSV must include a column named exactly "USDX".
#  - If your CSV uses different column names, edit get_features_for_model() or rename columns in the CSV.
...
MODEL_NAME <- "SENT-Bi-GRU"     # choose one from the list above
HORIZON    <- 3                # forecast horizon (3 trading days ahead)
LOOKBACK   <- 30               # input window length (timesteps)
EPOCHS     <- 30
BATCH_SIZE <- 32
SEED       <- 42
...
get_features_for_model <- function(model_name) {
  ...
  # All others: Brent + sentiment
  return(c("BRENT Close", "SENT"))
}

# ----------------------------
# Helpers: scaling, sequences
# ----------------------------
...
m_scaled <- metrics(y_test, pred_test)
...
out_metrics <- data.frame(
  ...
  lookback = LOOKBACK,
  horizon = HORIZON,
  MAE_scaled = unname(m_scaled["MAE"]),
  MSE_scaled = unname(m_scaled["MSE"]),
  RMSE_scaled = unname(m_scaled["RMSE"]),
  MAE_orig = unname(m_orig["MAE"]),
  MSE_orig = unname(m_orig["MSE"]),
  RMSE_orig = unname(m_orig["RMSE"])
)

metrics_path <- file.path(OUT_DIR, paste0("metrics_", MODEL_NAME, ".csv"))
preds_path   <- file.path(OUT_DIR, paste0("pred_test_", MODEL_NAME, ".csv"))
model_path   <- file.path(OUT_DIR, paste0("model_", MODEL_NAME, ".h5"))

write.csv(out_metrics, metrics_path, row.names = FALSE)

pred_df <- data.frame(
  y_true_scaled = y_test,
  y_pred_scaled = pred_test,
  y_true_orig = y_orig,
  y_pred_orig = pred_orig
)
write.csv(pred_df, preds_path, row.names = FALSE)

model |> save_model_hdf5(model_path)
